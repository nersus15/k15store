...
async function dataKeranjang() {
    const data = await fetch(path + '/api/keranjang/keranjang/' + session.username + "/null").then(res => res.json()).then(res => res);
    if (data.length > 0) {
        let rows = '';
        data.forEach((item, index) => {
            ...
            rows += `
                <tr>
                    <td id="detail" style="cursor: pointer" data-barang="${index}">${item.nama_product}</td>
                    <td>${item.jumlah}</td>
                    <td id="username" style="cursor: pointer" data-username="${item.owner}">${item.owner}</td>
        ...
        $("#detail").click(function () {
            ...
            barang.detail_alamat = !barang.detail_alamat ? `<span id="no-da" class="text-danger">Lengkapi alamat pengiriman</span>` : barang.detail_alamat;
            ...
            opt.saatBuka = () => {
                ...
                $('#edit').click(async function () {
                    ...
                    const customInput =
                        `   
                            <div class="custom-switch custom-switch-primary-inverse mb-2">
                                ...
                                <input class="custom-switch-input" id="langsung-bayar" type="checkbox" checked="">
                    ...
                    const inputs = [
                        ...
                        {
                            value: da, attr: 'required', type: 'textarea', id: 'detail_alamat', name: 'detail_alamat', label: 'Alamat pengiriman'
                        },
